function attachInvoiceListeners() {
    // ===============================================
    // 📎 Alle Buttons mit Klasse .invoiceBtn durchgehen
    ...
    document.querySelectorAll(".invoiceBtn").forEach(button => {
                button.addEventListener("click", () => {
                            ...
                            // 📨 Rechnungsdaten für diese Bestellung vom Backend holen
                            fetch(`/Snackery/Backend/logic/requestHandler.php?action=getInvoiceData&orderId=${orderId}`, {
                                    credentials: "include"
                                })
                                .then(res => res.json())
                                .then(data => {
                                        // Fehlerbehandlung: falls keine Daten zurückkommen
                                        if (!data.success) {
                                            alert("❌ Fehler beim Laden der Rechnungsdaten.");
                                            return;
                                        }

                                        // ✅ Erfolgreich: Daten entpacken
                                        ...
                                        // Neues Fenster für die Rechnung öffnen
                                        const invoiceWindow = window.open("", "Rechnung", "width=800,height=1000");

                                        // ✍️ HTML für die Rechnung dynamisch erzeugen
                    ...
                    // 🧾 Schließen des Schreibvorgangs im neuen Fenster
